...
void ClickGUI::onRender(Event&) {
	...
	RectF searchRect{};
	{
		...
		if (tab == SETTINGS) {
			...
			static std::vector<std::tuple<std::wstring, ClickGUI::ModTab, d2d::Color, float>> modTabs = { {L"All", ALL, searchCol, 0.f }, {L"Game", GAME, searchCol, 0.f }, {L"HUD", HUD, searchCol, 0.f }, { L"Plugins", SCRIPT, searchCol, 0.f }, { L"Illegal", ILLEGAL, searchCol, 0.f } };
			...
			if (searchTextBox.getText().empty()) {
				if (modTab == ALL) {
					...
					if (!mod.isMarketScript)
						continue;
				} else if (modTab == ILLEGAL) {
					if (!mod.mod || mod.mod->getCategory() != Module::ILLEGAL)
						continue;
				}
			} else {
