...
bool EspRenderer::getScreenRect(const CEntity::AABB& box, ImVec4& out) {
    ...
    for (int i = 0; i < 8; i++) {
        if (worldToScreen(corners[i].x, corners[i].y, corners[i].z, points[i])) {
            anyVisible = true;
    ...
    out = ImVec4(minX, minY, maxX, maxY);
    return true;
}

void EspRenderer::drawCornerBox(ImDrawList* drawList, float x, float y, float w, float h, float thickness, ImU32 color) {
    float lineW = w / 4.0f;
    float lineH = h / 4.0f;
...
ImU32 EspRenderer::getHealthColor(float percentage) {
    int r = (int)(255 * (1.0f - percentage));
